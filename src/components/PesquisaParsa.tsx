...
export function PesquisaParsa() {
  const [busca, setBusca] = useState<ParceiroProps[]>([]);
  ...
  async function updateStateParceiro(id: number) {
    ...
    try {
      const response = await api.put(`/parceiro/${id}`);
      JSON.stringify({
        id: Number,
        ativo: true,
      });
  ...
  async function updateStateParceiroBloqueio(id: number) {
    document.getElementById("bloqueio");
    addEventListener("submit", (e) => {
      e.preventDefault();
    });
    try {
      const response = await api.put(`/parceiroBloqueio/${id}`);
      JSON.stringify({
        id: Number,
        ativo: true,
      });
  ...
  async function updateDadosParceiro(event: FormEvent) {
    ...
    try {
      await api.post("/parceiro/update", {
        email: data.email,
        name: data.name,
        cnpj: data.cnpj,
  ...
  async function updateValorParceiro(id: number) {
    document.getElementById("configparceiro");
    addEventListener("submit", (e) => {
      e.preventDefault();
    });
    try {
  ...
  function handleInputChange(id: number) {
    if (id != null) {
      ...
      localStorage.setItem("idParceiro", idP);
    } else {
    }
  }
  ...
  async function deleteParceiro(id: number) {
    ...
    try {
      const response = await api.delete(`/parceiro/${id}`);
      JSON.stringify({
        id: Number,
      });
      alert("Tem certeza que vai excluir esse parceiro");
      return window.location.reload();
    } catch (error) {
      alert("algo deu errado");
    }
                  ...
                  <form className="flex justify-center items-center gap-4 ">
                          ...
                          case false:
                            return (
                              <button
                                ...
                                type="submit"
                                id="bloqueio"
                                title="Bloqueiar parceiro"
                                data-modal-toggle="editUserModal"
                                onClick={() => {
                                  updateStateParceiroBloqueio(busca.id);
                                }}
                    ...
                    <Dialog.Trigger asChild>
                      <button
                        id="configparceiro"
                        name="configparceiro"
                        className={`flex justify-center items-center  w-12 rounded px-2.5 py-2.5 text-write bg-cyan-700 rotate-90 `}
                        title="Configurar o valor que o parceiro paga por cliente"
                        value={busca.id}
                      >
                        <Money size={24} />
                      </button>
                            ...
                            <form className="relative bg-cyan-700 rounded-lg text-write p-6">
                              ...
                              <div className="flex justify-between items-start rounded-t  dark:border-gray-600">
                                <h3 className="text-xl font-semibold text-gray-900 text-write ml-4">
                                  Definir o valor por cliente pago pelo parceiro
                                </h3>
                                <button
                                  type="button"
